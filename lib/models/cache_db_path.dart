class CacheDBPath {
  static var prefix = 'db_cache_';
  static var myInfo = prefix + 'my_info';
  static var products = prefix + 'products';
  static var customers = prefix + 'customers';
  static var customersGroup = prefix + 'customers_group';
  static var warehouse = prefix + 'warehouse';
  // static var warehouseProducts = prefix + 'warehouse_products';
  static var warehouseProducts = prefix + 'warehouse_products_';
  static var customerPriceGroups = prefix + 'customer_price_groups';
}
